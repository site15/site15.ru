...
## 10. Паттерн реализации режима просмотра в компонентах таблиц

### Правило

Компоненты таблиц должны поддерживать режим просмотра (view mode) с возможностью отображения пользовательского заголовка вместо стандартных кнопок действий.

### Реализация

- Компоненты таблиц должны иметь входные параметры `viewMode` (boolean) и `title` (string)
- В режиме просмотра должны скрываться кнопки действий (создание, редактирование, удаление)
- В режиме просмотра вместо кнопки "Создать" отображается пользовательский заголовок
- В режиме просмотра не должны открываться модальные окна для создания/редактирования/удаления
- Пример реализации можно найти в компонентах модуля metrics-afat

## 11. Согласованность реализации функциональности между модулями

### Правило

Однотипная функциональность в разных модулях должна реализовываться единообразно.

### Реализация

- При добавлении новой функциональности в один модуль, аналогичная функциональность должна быть добавлена в другие модули
- Паттерны реализации должны быть согласованы между модулями
- Пример: функциональность режима просмотра была добавлена в модуль metrics-afat и затем реализована аналогично в модуле sso-afat
