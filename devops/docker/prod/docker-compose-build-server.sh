...
if [ -z "${NO_PUSH_DOCKER_IMAGES}" ]; then
    if [ -z "${CI_COMMIT_SHORT_SHA}" ]; then
        ...
        export IMG="${CI_REGISTRY}/${GITHUB_REPOSITORY_OWNER}/${SERVER_BASE_IMAGE}"
        ...
        docker build --platform linux/amd64 -t ${CI_REGISTRY}/${GITHUB_REPOSITORY_OWNER}/${SERVER_IMAGE} . --file ./docker/prod/Dockerfile/server-image.Dockerfile
        docker tag ${CI_REGISTRY}/${GITHUB_REPOSITORY_OWNER}/${SERVER_IMAGE} ${CI_PROJECT_NAME}-${TAG_VERSION}
    else
        ...
        docker build --platform linux/amd64 -t ${CI_REGISTRY}/${GITHUB_REPOSITORY_OWNER}/${SERVER_IMAGE} . --file ./docker/prod/Dockerfile/server-image.Dockerfile
        docker push ${CI_REGISTRY}/${GITHUB_REPOSITORY_OWNER}/${SERVER_IMAGE}
        docker tag ${CI_REGISTRY}/${GITHUB_REPOSITORY_OWNER}/${SERVER_IMAGE} ${CI_PROJECT_NAME}-${TAG_VERSION}
    fi
else
