...
import { MetricsGithubUserModel } from '../../services/metrics-github-user-mapper.service';
import { MetricsGithubUserFormComponent } from '../../forms/metrics-github-user-form/metrics-github-user-form.component';
...
export class MetricsGithubUserGridComponent implements OnInit {
  ...
  items$ = new BehaviorSubject<MetricsGithubUserModel[]>([]);
  meta$ = new BehaviorSubject<RequestMeta | undefined>(undefined);
  searchField = new FormControl('');
  selectedIds$ = new BehaviorSubject<string[]>([]);

  keys = [
    MetricsGithubUserScalarFieldEnumInterface.id,
    MetricsGithubUserScalarFieldEnumInterface.login,
    MetricsGithubUserScalarFieldEnumInterface.name,
    MetricsGithubUserScalarFieldEnumInterface.email,
  ];

  columns = {
    [MetricsGithubUserScalarFieldEnumInterface.id]: marker('metrics-github-user.grid.columns.id'),
    [MetricsGithubUserScalarFieldEnumInterface.login]: marker('metrics-github-user.grid.columns.login'),
    [MetricsGithubUserScalarFieldEnumInterface.name]: marker('metrics-github-user.grid.columns.name'),
    [MetricsGithubUserScalarFieldEnumInterface.email]: marker('metrics-github-user.grid.columns.email'),
  };

  MetricsGithubUserScalarFieldEnumInterface = MetricsGithubUserScalarFieldEnumInterface;

  private filters?: Record<string, string>;

  constructor(
    private readonly metricsGithubUserService: MetricsGithubUserService,
  ...
  loadMany(args?: {
    ...
    this.metricsGithubUserService
      .findMany({ filters, meta })
      .pipe(
        tap((result) => {
          this.items$.next(result.metricsGithubUsers);
  ...
  showCreateOrUpdateModal(id?: string): void {
    ...
    const modal = this.nzModalService.create<MetricsGithubUserFormComponent, MetricsGithubUserFormComponent>({
      nzTitle: id
        ? this.translocoService.translate('metrics-github-user.update-modal.title', {
            id,
          })
        : this.translocoService.translate('metrics-github-user.create-modal.title'),
      nzContent: MetricsGithubUserFormComponent,
      nzViewContainerRef: this.viewContainerRef,
      nzData: {
        hideButtons: true,
        id,
      } as MetricsGithubUserFormComponent,
      nzFooter: [
        {
  ...
  showDeleteModal(id?: string) {
    ...
    this.nzModalService.confirm({
      nzTitle: this.translocoService.translate(`metrics-github-user.delete-modal.title`, {
        id,
      }),
