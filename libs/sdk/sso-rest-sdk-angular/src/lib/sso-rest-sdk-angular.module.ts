...
import { SsoRestClientApiModule, SsoRestClientConfiguration } from './generated';

@NgModule({})
export class SsoRestSdkAngularModule {
  public static forRoot(configuration: Partial<SsoRestClientConfiguration>) {
    const ssoRestClientConfiguration = new SsoRestClientConfiguration(configuration);
    const ssoRestClientApiModule = SsoRestClientApiModule.forRoot(() => ssoRestClientConfiguration);
    return {
